$(document).ready(function ready() {
  /*============================================
	Page Preloader
	==============================================*/

  $(window).load(() => {
    $("#page-loader").fadeOut(500, () => {});
  });

  /*============================================
	Header
	==============================================*/

  $("#home").height($(window).height() + 50);

  $.backstretch("/images/header-bg.jpg");

  $(window).scroll(function scrollTop() {
    const st = $(this).scrollTop();
    const wh = $(window).height();
    const sf = 1.2 - st / (10 * wh);

    $(".backstretch img").css({
      transform: `scale(${sf})`,
      "-webkit-transform": `scale(${sf})`,
    });

    $("#home .container").css({ opacity: 1.4 - st / 400 });

    if ($(window).scrollTop() > $(window).height() + 50) {
      $(".backstretch").hide();
    } else {
      $(".backstretch").show();
    }
  });

  const st = $(this).scrollTop();
  const wh = $(window).height();
  const sf = 1.2 - st / (10 * wh);

  $(".backstretch img").css({
    transform: `scale(${sf})`,
    "-webkit-transform": `scale(${sf})`,
  });

  $("#home .container").css({ opacity: 1.4 - st / 400 });

  /*============================================
	Navigation Functions
	==============================================*/
  if ($(window).scrollTop() < $(window).height() - 50) {
    $("#main-nav").removeClass("scrolled");
  } else {
    $("#main-nav").addClass("scrolled");
  }

  $(window).scroll(() => {
    if ($(window).scrollTop() < $(window).height() - 50) {
      $("#main-nav").removeClass("scrolled");
    } else {
      $("#main-nav").addClass("scrolled");
    }
  });

  /*============================================
	ScrollTo Links
	==============================================*/
  // eslint-disable-next-line func-names
  $("a.scrollto").click(function (e) {
    $("html,body").scrollTo(this.hash, this.hash, { gap: { y: -70 } });
    e.preventDefault();

    if ($(".navbar-collapse").hasClass("in")) {
      $(".navbar-collapse").removeClass("in").addClass("collapse");
    }
  });

  /*============================================
	Skills
	==============================================*/
  $(".skills-item").each(function percentage() {
    const perc = $(this).find(".percent").data("percent");

    $(this).data("height", perc);
  });

  $(".touch .skills-item").each(function skillsItem() {
    $(this).css({ height: `${$(this).data("height")}%` });
  });

  $(".touch .skills-bars").css({ opacity: 1 });

  /*============================================
	Project thumbs - Masonry
	==============================================*/
  $(window).load(() => {
    $("#projects-container").css({ visibility: "visible" });

    $("#projects-container").masonry({
      itemSelector: ".project-item:not(.filtered)",
      //columnWidth:370,
      isFitWidth: true,
      isResizable: true,
      isAnimated: !Modernizr.csstransitions,
      gutterWidth: 25,
    });

    scrollSpyRefresh();
    waypointsRefresh();
  });

  /*============================================
	Filter Projects
	==============================================*/
  $("#filter-works a").click(function filterProjects(e) {
    e.preventDefault();

    if ($("#project-preview").hasClass("open")) {
      closeProject();
    }

    $("#filter-works li").removeClass("active");
    $(this).parent("li").addClass("active");

    const category = $(this).attr("data-filter");

    $(".project-item").each(function filterCategory() {
      if ($(this).is(category)) {
        $(this).removeClass("filtered");
      } else {
        $(this).addClass("filtered");
      }

      $("#projects-container").masonry("reload");
    });

    scrollSpyRefresh();
    waypointsRefresh();
  });

  /*============================================
	Project Preview
	==============================================*/
  $(".project-item").click(function previewProject(e) {
    e.preventDefault();

    const elem = $(this);
    const title = elem.find(".project-title").text();
    const descr = elem.find(".project-description").html();
    let slidesHtml = '<ul class="slides">';
    const elemDataCont = elem.find(".project-description");

    const slides = elem.find(".project-description").data("images").split(",");

    for (const slide of slides) {
      slidesHtml = `${slidesHtml}<li><img src=${slide} alt="trendy table and chairs background"></li>`;
    }

    slidesHtml = `${slidesHtml}</ul>`;

    $("#project-title").text(title);
    $("#project-content").html(descr);
    $("#project-slider").html(slidesHtml);

    openProject();
  });

  function openProject() {
    $("#project-preview").addClass("open");
    $(".masonry-wrapper").animate({ opacity: 0 }, 300);

    setTimeout(() => {
      $("#project-preview").slideDown();
      $(".masonry-wrapper").slideUp();

      $("html,body").scrollTo(0, "#filter-works", {
        gap: { y: -20 },
        animation: {
          duration: 400,
        },
      });

      $("#project-slider").flexslider({
        prevText: '<i class="fa fa-angle-left"></i>',
        nextText: '<i class="fa fa-angle-right"></i>',
        animation: "slide",
        slideshowSpeed: 3000,
        useCSS: true,
        controlNav: true,
        pauseOnAction: false,
        pauseOnHover: true,
        smoothHeight: false,
        start() {
          $(window).trigger("resize");
          $("#project-preview").animate({ opacity: 1 }, 300);
        },
      });
    }, 300);
  }

  function closeProject() {
    $("#project-preview").removeClass("open");
    $("#project-preview").animate({ opacity: 0 }, 300);

    setTimeout(() => {
      $(".masonry-wrapper").slideDown();
      $("#project-preview").slideUp();

      $("#project-slider").flexslider("destroy");
      $(".masonry-wrapper").animate({ opacity: 1 }, 300);
    }, 300);

    setTimeout(() => {
      $("#projects-container").masonry("reload");
    }, 500);
  }

  $(".close-preview").click(() => {
    closeProject();
  });

  /*============================================
	Waypoints Animations
	==============================================*/
  $("#skills").waypoint(
    () => {
      $(".skills-item").each(function skills() {
        $(this).css({ height: `${$(this).data("height")}%` });
      });

      $(".skills-bars").css({ opacity: 1 });
    },
    { offset: "40%" }
  );

  $(".scrollimation").waypoint(
    function scroll() {
      $(this).addClass("in");
    },
    { offset: "90%" }
  );

  /*============================================
	Resize Functions
	==============================================*/
  let thumbSize = $(".project-item").width();

  $(window).resize(() => {
    $("#home").height($(window).height() + 50);

    if ($(".project-item").width() !== thumbSize) {
      $("#projects-container").masonry("reload");
      thumbSize = $(".project-item").width();
    }

    scrollSpyRefresh();
    waypointsRefresh();
  });

  /*============================================
	Refresh scrollSpy function
	==============================================*/
  function scrollSpyRefresh() {
    setTimeout(() => {
      $("body").scrollspy("refresh");
    }, 1000);
  }

  /*============================================
	Refresh waypoints function
	==============================================*/
  function waypointsRefresh() {
    setTimeout(() => {
      $.waypoints("refresh");
    }, 1000);
  }
});
